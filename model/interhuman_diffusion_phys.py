...
from utils.imutils import cam_crop2full, vis_img, cam_full2crop
...
class interhuman_diffusion_phys(interhuman_diffusion):
    def __init__(self, smpl, use_classifier_guidance=False, scale_factor = 1000000.0, **kwargs):
        ...
        #     from model.interprior.inter_pose_vqvae import InterHumanTokenizer
        ...
        if self.use_classifier_guidance or self.use_classifier_guidance_mean or self.use_classifier_guidance_cond:
            ...
            state_dict = torch.load('/root/autodl-tmp/PW3D/out/refine_joints_prior64/04.13-23h35m35s/trained model/best_reconstruction_j3d_epoch039_1341784.500000.pkl')['model']
            # remove the prefix 'model.'
            # new_state_dict = {}
            # for k, v in state_dict.items():
            #     new_state_dict[k[6:]] = v
    ...
    def classifier_guidance(self, x_t, data, img_info, t=0, i=0, debug=False):
        ...
        gt_joints = data['gt_joints'][...,:3].reshape(-1, 26, 3)
        gt_trans = data['gt_cam_t'].reshape(-1, 3)
        gt_joints = gt_joints + gt_trans[..., None, :]
        gt_joints = gt_joints.reshape(batch_size, frame_length, agent_num, 26, 3)
        gt_joitns_a = gt_joints[:, :, 0]
        gt_joints_b = gt_joints[:, :, 1]
        joint_dist_mat_ab = torch.cdist(gt_joitns_a, gt_joints_b, p=2) # [bs, seq_len, 26, 26]
        joint_dist_mat_ab = joint_dist_mat_ab.mean(dim=-1) # [bs, seq_len, 26]
        joint_dist_mat_ba = torch.cdist(gt_joints_b, gt_joitns_a, p=2) # [bs, seq_len, 26, 26]
        joint_dist_mat_ba = joint_dist_mat_ba.mean(dim=-1) # [bs, seq_len, 26]
        ...
        with torch.enable_grad():
            ...
            def closure_trans():
                ...
                pred_joints_halpe = pred_joints_halpe.reshape(batch_size, frame_length, agent_num, 26, 3)
                target_g_joints_ = target_g_joints.reshape(batch_size, frame_length, agent_num, 26, 3)
                joint_loss = loss_fn(pred_joints_halpe, target_g_joints_).sum(dim=-1) # [bs, seq_len, num_person, 26, 3]
                # set loss to zero if joint_dist_mat is too large
                joint_dist_mat = torch.stack([joint_dist_mat_ab, joint_dist_mat_ba], dim=2) # [bs, seq_len, 2, 26]
                joint_loss = torch.where(joint_dist_mat < 0.1, joint_loss, torch.zeros_like(joint_loss))
    ...
    def classifier_guidance_cond(self, img, t, mean, cond, img_info, data, debug = False, **kwargs):
        ...
        with torch.enable_grad():
            ...
            def closure():
                ...
                # torch.nn.utils.clip_grad_norm_([cond], 0.01)
    ...
    def inference(self, x_t, t, cond, img_info, data, mean, **kwargs):
        ...
        valid_sum = data['valid'].sum()
        if self.use_classifier_guidance and not self.training and t[0,0,0] < self.num_timesteps * 0.2 and valid_sum > 0:
        ...
        output_init = True
        if output_init and not self.training:
