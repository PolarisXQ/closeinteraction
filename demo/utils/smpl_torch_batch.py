...
class SMPLModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
                 data_type=torch.float32, simplify=False):
        ...
        self.J_regressor = torch.from_numpy(np.array(params['J_regressor'].todense())).to(self.data_type).to(self.device)
        self.J_halpe_regressor = torch.from_numpy(np.load('smpl/J_regressor_halpe.npy')).to(self.data_type).to(self.device)
        self.J_lsp_regressor = torch.from_numpy(np.load('smpl/J_regressor_lsp.npy')).to(self.data_type).to(self.device)
        self.weights = torch.from_numpy(params['weights']).to(self.data_type).to(self.device)
        self.posedirs = torch.from_numpy(params['posedirs']).to(self.data_type).to(self.device)
        self.v_template = torch.from_numpy(params['v_template']).to(self.data_type).to(self.device)
        self.shapedirs = torch.from_numpy(np.array(params['shapedirs'])).to(self.data_type).to(self.device)
        ...
        # add torch param
        mean_pose = np.zeros((72,))
        mean_shape = np.zeros((10,))
        ...
        for name in ['J_regressor', 'J_halpe_regressor', 'weights', 'posedirs', 'v_template', 'shapedirs']:
            _tensor = getattr(self, name)
            #print(' Tensor {} shape: '.format(name), _tensor.shape)
            self.register_buffer('name', _tensor)
    ...
    def forward(self, betas, thetas, trans, gR=None, halpe=False, lsp=False):
        ...
        if halpe:
            joints = torch.tensordot(result, self.J_halpe_regressor.transpose(0, 1), dims=([1], [0])).transpose(1, 2)
        elif lsp:
            joints = torch.tensordot(result, self.J_lsp_regressor.transpose(0, 1), dims=([1], [0])).transpose(1, 2)
        else:
