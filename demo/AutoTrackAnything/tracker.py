...
class Tracker:
    def __init__(self, xmem_config, max_obj_cnt, device):
        ...
        if self.device.lower() != 'cpu':
            self.network = XMem(
                self.xmem_config, 'pretrained/AutoTrackAnything_data/XMem.pth').eval().to('cuda')
        else:
            self.network = XMem(
                self.xmem_config, 'pretrained/AutoTrackAnything_data/XMem.pth', map_location='cpu').eval()
        ...
        self.colors = [(1,1,1), (2,1,1), (3,1,1), (4,1,1), (5,1,1), (6,1,1), (7,1,1), (8,1,1), (9,1,1), (10,1,1), (11,1,1), (12,1,1), (13,1,1), (14,1,1), (15,1,1), (16,1,1), (17,1,1)]
    ...
    def masks_on_im(self, masks, image):
        ...
        for i, mask in enumerate(masks):
            if i > len(self.colors) - 1:
                ...
                color = self.colors[i] #np.random.randint(0, 256, size=3)
    ...
    def create_mask_from_img(self, image, yolov7_bboxes, sam_predictor):
        ...
        mask_layers = [mask.cpu().squeeze().numpy().astype(np.uint8) for mask in masks]
