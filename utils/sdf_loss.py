...
class SDFLoss(nn.Module):
    ...
    def forward(self, vertices, translation, scale_factor=0.2, iter=0, threshold=0.0):
        num_people = vertices.shape[0]
        ...
        overlapping_boxes = ~self.filter_isolated_boxes(boxes).bool()
        ...
        if debug:
            ...
            n=0
            op = f"inspect_out/compute_metrics/sdf/loss"
            save_mesh(vertices_centered_scaled, faces=self.faces.cpu(), out_path=f"{op}/meshes_cent_scaled_{n}.ply")
            save_mesh(vertices, faces=self.faces.cpu(), out_path=f"{op}/meshes_orig_{n}.ply")
        ...
        for i in range(valid_people):
            ...
            verts_local.append(vertices_local.cpu())
            ...
            losses.append(cur_loss[weights[:, 0].bool()].cpu())
            if self.debugging:
            ...
            idxs_th = cur_loss > threshold
            sdf_in_vals_th = cur_loss[idxs_th]
            loss += sdf_in_vals_th.sum() / valid_people
